    ...
    func doneAction() {
        ...
        let action = UIAlertAction(title: "Сохранить", style: .default, handler: { action -> Void in
            ...
            if self.editImage != nil && self.index != nil {
                let pic = MainView.pictures[self.index!.row]
                pic.set(image: drawImage, name: sketchName)
                MainView.pushDrawingcVC()
            } else {
                MainView.pictures.insert(Pictures(with: drawImage, name: sketchName), at: 1)
            }
